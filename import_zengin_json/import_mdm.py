...
def import_model_mesh_from_json(mdm_json_data, texture_folder_list, rename_bones=False, add_root_bone=False):
    ...
    if len(model_mesh_dict['meshes']) > 0:
        assert 'multi_resolution_mesh' in model_mesh_dict['meshes'][0]

        multi_resolution_mesh = model_mesh_dict['meshes'][0]['multi_resolution_mesh']
        ...
        if armature_obj:
            ...
            soft_skin_weight_list = model_mesh_dict['meshes'][0]['soft_skin_weight']
...
def load_from_gothic_hub_scripts(config_file_path):
    ...
    for mdm_json_file_path in mdm_json_file_path_list:
        file_name = mdm_json_file_path.stem.upper().replace('.JSON', '')
        ...
        # if 'ARMOR_VLK_H' not in str(mdm_json_file_path):
        #     continue
        ...
        # if 'Addon' not in str(mdm_json_file_path):  # Addon, Gothic II,
        #     continue
        #
        # if 'CHESTBIG_ADD_STONE_LOCKED' not in str(mdm_json_file_path):  # CHESTBIG_ADD_STONE_LOCKED
        #     continue
        ...
        import_model_mesh_from_json(mdm_json_dict, texture_folder_list, rename_bones=config['rename_bones'], add_root_bone=config['add_root_bone'])
